...
class DVR(object):
    ...
    def sho_test(self, k=1., num_eigs=5, x0=0., precision=8, 
    ...
    def morse_test(self, D=3., a=0.5, num_eigs=5, x0=0., precision=8):
       # print 'Testing 1-D DVR with a Morse potential'
        vF = VFactory()
        V = vF.morse(D=D, a=a, x0=x0)
    ...
    def sombrero_test(self, a=-5.0, b=1.0, num_eigs=5, precision=8):
        #print 'Testing 1-D DVR with a sombrero potential'
        vF = VFactory()
        V = vF.sombrero(a=a, b=b)
        self.test_potential(V, num_eigs=num_eigs, precision=precision,
                            xmin=-5., xmax=5., ymax=5.)
        print
        return

    def woods_saxon_test(self, V0=50.0, z=0.5, r0=1.2, A=16, num_eigs=5, precision=8):
        #print 'Testing 1-D DVR with a Woods-Saxon potential'
        vF = VFactory()
        V = vF.woods_saxon(V0=V0, z=z, r0=r0, A=A)
        self.test_potential(V, num_eigs=num_eigs, precision=precision,
                            xmin=0., xmax=5., 
                            ymin=-50., ymax=0.)
        print
