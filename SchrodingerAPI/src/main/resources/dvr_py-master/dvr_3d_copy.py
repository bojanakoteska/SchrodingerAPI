...
#                      uscale=uscale, doshow=doshow)
    ...
    def sho_test(self, k = 1., num_eigs=5, x0=0., y0=0., z0=0, precision=8, 
                 uscale=1., doshow=False, do_full_eig=False):
       # print 'Testing 3-D DVR with an SHO potential'
        vF = VFactory()
        V = vF.sho(k=k, x0=x0, y0=y0, z0=z0)
        self.test_potential(V, doshow=doshow, num_eigs=num_eigs, 
        ...
        print
        return
    def morse_test(self, D1=3., a1=0.5, D2=3., a2=0.5, D3=3., a3=0.5, num_eigs=5, x0=0., y0=0., z0=0., precision=8,  uscale=1., doshow=False,  do_full_eig=False):
        #print 'Testing 3-D DVR with a Morse potential'
        vF = VFactory()
        V = vF.morse(D1=D1, a1=a1, x0=x0, D2=D2, a2=a2, y0=y0, D3=D3, a3=a3, z0=z0)
        self.test_potential(V, doshow=doshow, num_eigs=num_eigs, 
                                   precision=precision, uscale=uscale,
                                   xmin=-3.5, xmax=3.5, 
                                   ymin=-3.5, ymax=3.5,
...
class VFactory(object):
    ...
    def morse(self, D1 = 1., a1 = 1., x0 = 0., D2 = 1., a2 = 1., y0 = 0., D3 = 1., a3 = 1., z0 = 0.,):
        """Usage:
        ...
        """
        def V(xyz): 
            return D1 * np.power(1. - np.exp(-a1 * (xyz[:,0] - x0)), 2.) - D1+ D2 * np.power(1. - np.exp(-a2 * (xyz[:,1] - y0)), 2.) - D2 + D3 * np.power(1. - np.exp(-a3 * (xyz[:,2] - z0)), 2.) - D3
        return V
